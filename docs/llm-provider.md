...
Provider envelopes are normalized before evaluation parsing. The server accepts the common `choices[0].message.content`, Ollama-compatible content arrays, `response`, and `output_text` shapes. Assistant content may include a short prose wrapper, a fenced JSON object, double-encoded JSON, or a single `evaluation`/`result` envelope. The parser extracts exactly one complete balanced object; ambiguous or incomplete content fails validation. Scores are normalized from `[0,1]`, numeric `[0,100]`, percentage strings, or decimal strings, while ambiguous and out-of-range values fail validation. Verdicts, error types, and severities are normalized to canonical enums; `errors: null` becomes `[]`.

An invalid structured result gets up to two bounded repair requests with explicit JSON-only instructions. Provider failures are classified as timeout, 4xx, 5xx, envelope, or structured-output failures. Attempts retain a safe diagnostic record containing request ID, provider type, model, status, latency, failure stage, response shape, retry count, and schema error. Secrets and Authorization headers are never persisted.

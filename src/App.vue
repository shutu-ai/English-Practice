...
async function submit(){ if(!exercise.value || !answer.value.trim()) return; loading.value=true; message.value=''; try{const result=await api<{evaluation_status:string;evaluation?:Evaluation;error?:string}>('/api/attempts',{method:'POST',body:JSON.stringify({exercise_id:exercise.value.exercise_id,answer:answer.value})}); if(result.evaluation) evaluation.value=result.evaluation; else message.value=result.error||'评估失败'}catch(e){message.value=(e as Error).message}finally{loading.value=false} }
